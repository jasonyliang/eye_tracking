...
while(cap.isOpened()):
	...
	if ret==True:
		...
		for (x,y,w,h) in detected:
			...
			if len(contours) >= 2:      #get largest blob
				...
				secondLargest = 0
				secondIndex = None
				for indx, a in enumerate(areas):
					if a == largestA:
						largeBlob.append(contours[indx])
					elif a > secondLargest:
						secondLargest = a
						secondIndex = indx
				largeBlob.append(contours[secondIndex])
			if largeBlob and len(largeBlob) > 0: 
				for blob in largeBlob: 
					center = cv2.moments(blob)
					cx,cy = int(center['m10']/center['m00']), int(center['m01']/center['m00'])
					cv2.circle(pupilO,(cx,cy),5,255,-1)
				# print location
				#print(cx,cy)
